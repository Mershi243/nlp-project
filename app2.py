...
def preprocess_text(text):
    if not isinstance(text, str) or text.strip() == "":
        return ""  # Handle empty or non-string cases

    nltk.data.find("tokenizers/punkt")  # Ensure 'punkt' is available
...
def analyze_sentiment(text):
    ...
    if processed_text.strip() == "":
        return text, 0.0, "Neutral"  # Handle empty processed text gracefully
